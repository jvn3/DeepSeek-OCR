...
class DeepseekOCRProcessor(ProcessorMixin):
    ...
    def process_one(
        ...
        **kwargs,
        ...
        # Check if images[0] is a PIL Image (raw) or already processed (tuple)
        if isinstance(images[0], Image.Image):
            # Raw PIL Image from vLLM - need to preprocess it
            processed = self.tokenize_with_images(
                images=images,
                bos=True,
                eos=True,
                cropping=True
            )
            input_ids, pixel_values, images_crop, images_seq_mask, images_spatial_crop, num_image_tokens, _ = processed[0]
        else:
