    ...
    def _get_prompt_updates(
        ...
        def get_replacement_deepseek_vl2(item_idx: int):
            ...
            if isinstance(images, ImageEmbeddingItems):
                ...
                # Get the PIL Image from ImageProcessorItems
                from PIL import Image
                pil_image = images[0]
                
                # Check if it's a raw PIL Image or pre-processed tuple
                if isinstance(pil_image, Image.Image):
                    # Raw PIL Image - get dimensions directly
                    width, height = pil_image.size
                else:
                    # Pre-processed tuple - extract from image_shapes (last element)
                    width = pil_image[-1][0][0]
                    height = pil_image[-1][0][1]
